...
Logger::Log::Log(LogLevel loglevel, const std::string &msg) noexcept {
  ...
  stream << std::put_time(&tm, "[%d %b %Y %H:%M:%S]")
         << log_level_name[static_cast<int>(loglevel)] << msg << std::endl;
...
Logger::Logger(
  ...
  log_writer_ = std::thread(&Logger::LogWriting,
                            this); // log_writer_线程函数为Logger::LogWriting
}
...
Logger::~Logger() {
  ...
  cv_.notify_one();
  if (log_writer_
          .joinable()) { // 日志write线程和Logger线程还有关系，则log线程负责回收log_writer_
    log_writer_.join();
  }
}
