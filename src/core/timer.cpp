...
auto Timer::AddSingleTimer(uint64_t expire_from_now, const std::function<void()> &callback) noexcept -> SingleTimer * {
    ...
    if (new_next_expire != next_expire_) {
        ...
        ResetTimerFd(timer_fd_, FromNowInTimeSpec(next_expire_)); //添加新的timer后，下次超时时间变了，修改为新的
    }
    return raw_timer;
}
...
auto Timer::RemoveSingleTimer(SingleTimer *single_timer) noexcept -> bool {
    ...
    if (it != timer_queue_.end()) {
        ...
        if (new_next_expire != next_expire_) {
            next_expire_ = new_next_expire;
            ResetTimerFd(timer_fd_, FromNowInTimeSpec(next_expire_)); //下次超时时间变了，修改为新的
        }
        return true;
    }
    return false;
...
auto Timer::RefreshSingleTimer(SingleTimer *single_timer, uint64_t expire_from_now) noexcept -> Timer::SingleTimer * {
    std::unique_lock<std::mutex> lock(mtx_);
    auto it = timer_queue_.find(single_timer);
    if (it == timer_queue_.end()) {
        return nullptr;   
...
auto Timer::PruneExpiredTimer() noexcept -> std::vector<std::unique_ptr<SingleTimer>> {
    ...
    if (new_next_expire != next_expire_) {
        next_expire_ = new_next_expire;
        ResetTimerFd(timer_fd_, FromNowInTimeSpec(next_expire_)); //移出超时的定时器，下次超时时间变了，修改为新的
    }
    return expired;
}
